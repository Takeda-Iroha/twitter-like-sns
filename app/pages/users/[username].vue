...
const loadUserPosts = async () => {
  ...
  try {
    ...
    if (isMyPage.value) {
      ...
      userPosts.value = result.posts.filter(p => p.visibility === 'public' || p.visibility === 'followers')
    } else {
...
<template>
  <div class="userpage-content">
    ...
    <div class="mypage-wrapper">
      ...
      <template v-else-if="profile">
        ...
        <section class="profile-header">
          ...
          <div class="profile-main">
            ...
            <div class="profile-action">
              <button v-if="isMyPage" class="edit-btn" @click="navigateTo('/mypage')">
                プロフィールを編集
              </button>
              <button
                v-else
                class="follow-btn"
                :class="{ 'following': followStatus === 'following', 'pending': followStatus === 'pending' }"
        ...
        <main class="post-list">
          ...
          <article v-else v-for="post in userPosts" :key="post.id" class="post-card" @click="navigateTo(`/posts/${post.id}`)">
            <div class="post-inner">
              <div class="post-header">
                <div class="icon-wrapper" @click.stop="navigateTo(`/users/${post.author.username}`)">
                  <UserAvatar :profile-image-url="post.author.profileImageUrl" :size="40" />
                </div>
    ...
    <!-- フォロー解除確認モーダル -->
    <div v-if="showUnfollowModal" class="modal-overlay" @click.self="showUnfollowModal = false">
      <div class="modal">
        <p class="modal-message">{{ profile?.displayName || profile?.username }} さんのフォローを解除しますか？</p>
        <p v-if="unfollowError" class="unfollow-error">{{ unfollowError }}</p>
        <div class="modal-buttons">
          <button class="modal-cancel-btn" @click="showUnfollowModal = false">キャンセル</button>
          <button class="modal-unfollow-btn" :disabled="isUnfollowing" @click="handleUnfollow">
...
.modal-message { font-size: 16px; font-weight: bold; margin: 0 0 20px; color: #333; line-height: 1.5; }
.unfollow-error { color: #f66; font-size: 13px; margin: 0 0 12px; }
...
.modal-unfollow-btn { flex: 1; background-color: #f66; color: white; border: none; border-radius: 20px; padding: 10px; font-size: 14px; font-weight: bold; cursor: pointer; }
